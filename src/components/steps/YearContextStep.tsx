    ...
    <div className="animate-fade-up max-w-2xl mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-slate-400 text-lg max-w-xl mx-auto">
          Jak ten kwartał wpływa na <span className="text-ember-400 font-medium">Twój cel roczny {year}</span>?
        </p>
      </div>

      {/* Year context visual */}
      <div className="mb-8 p-6 bg-night-800/30 rounded-2xl border border-night-700/50">
        <div className="flex items-center justify-between mb-4">
          <span className="text-sm text-slate-400">Rok {year}</span>
          <span className="text-sm text-ember-400">Twój kwartał</span>
        </div>
        <div className="flex gap-2">
          {['Q1', 'Q2', 'Q3', 'Q4'].map((q) => (
            <div
              key={q}
              className="flex-1 h-3 rounded-full bg-night-700 overflow-hidden"
            >
              <div
      ...
      <div className="relative">
        <textarea
          ...
          className="w-full h-36 p-6 bg-night-800/50 border border-night-700 rounded-2xl text-white text-lg placeholder-slate-500/50 resize-none transition-all duration-300 focus:border-ember-500/50 focus:ring-2 focus:ring-ember-500/20"
        />

        {/* Character count */}
        <div className="absolute bottom-4 right-4 text-sm">
          <span className={remaining < 50 ? 'text-ember-400' : 'text-slate-500'}>
            {remaining}
          </span>
          <span className="text-slate-600"> znaków</span>
        </div>
      </div>

      {/* Completion celebration */}
      <div className="mt-8 p-6 bg-gradient-to-br from-ember-500/10 to-indigo-500/10 rounded-2xl border border-ember-500/20">
      ...
      <div className="mt-6 p-4 bg-night-800/30 rounded-xl border border-night-700/50">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="text-sm font-medium text-slate-300 mb-1">Perspektywa długoterminowa</h4>
